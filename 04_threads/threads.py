...
def addMailToThreadById(mail, threads, threadId):
	...
	if not mail['_id'] in mails:
		...
		title = currentThread['title'] if len(currentThread['title']) > normalize(mail['subject']) else normalize(mail['subject'])
		...
		threads.update({'_id':threadId},{'$push':{'mails':mail['_id']},'$inc':{'count':1},'$set':{'startdate':startdate,'enddate':enddate,'duration':duration}})
...
def addMailToThreads(mail, threads, threshold, threadDuration, minimalLengthSubject):
	...
	for thread in threads.find({'startdate':{'$gt':sup},'enddate':{'$lt':inf}}):
		if (thread['title'] != '' and SequenceMatcher(None, title, thread['title']).ratio() >= threshold) and (abs(mail['timestamp'] - thread['startdate']) <= threadDuration):
			...
			return threadId
		elif (len(title) >= minimalLengthSubject) and (len(thread['title']) >= minimalLengthSubject) and (title in thread['title']):
			log(f, 'debug', 'Title to merge :\t\t' + title + '\t\t\tinto\t\t\t' + thread['title'])
...
# minimalLengthSubject 		: int 			: minimal number of characters of the normalized title to check if it is contains into a thread title
# return 					: void
def addMailsToThreads(mails, threads, threshold, threadDuration, minimalLengthSubject):
	threadId = addMailToThreads(mails.pop(0), threads, threshold, threadDuration, minimalLengthSubject)
	for mail in mails:
		addMailToThreadById(mail, threads, threadId)
	...
	for mail in mails.find(timeout=False):
		...
		if xreference != '':
			...
			if mails.find({'X-Message-Id':xreference}).count() == 1:
				...
				addMailsToThreads([origin, mail], threads, threshold, threadDuration, minimalLengthSubject)
				continue
			else:
				count += 1
				log(f, 'warning', 'No referenced message with id : ' + xreference)
		# 2. Add this mail to a thread
		addMailsToThreads([mail], threads, threshold, threadDuration, minimalLengthSubject)
